...
class ZZSpiderOld(CrawlSpider):
    ...
    def parse_page(self, response):
        ...
        try:
            item['date'] =  response.xpath("//div[@id='article']/table/tr[1]/td[2]/p/text()").extract()[0].split(":")[1]
        except IndexError:
            item['date'] = ""
        try:
            item['comment_count'] = int(response.xpath("//div[@id='article']/table/tr[1]/td[1]/a/text()").extract()[0].split("(")[1].split(")")[0])
        except IndexError:
            item['comment_count'] = ""
        yield item
